...
from playlist.utils import append_track_data
...
def authorize_with_spotify(request, spotify_code=None):
    ...
    context = {"current_user_playlists": current_user_playlists}
...
def add_playlist_to_room(request, playlist_id):
    ...
    if Playlist.objects.filter(room=room).filter(playlist_id=playlist_id).count() == 0:
        ...
        playlist = Playlist.objects.get(
            room=room, created_by=request.user, playlist_id=playlist_id
        )
    ...
    for track in user_playlist_tracks["items"]:
        if (
            Track.objects.filter(playlist=playlist)
            .filter(track_id=track["track"]["id"])
            .count()
            == 0
        ):
...
def search_track_name(request, playlist_id):
    ...
    if request.method == "POST":
            ...
            while ((NEXT != "null") | (NEXT != None)) & (OFFSET < OFFSET_UPPER_LIMIT):
                try:
                    ...
                    if TOTAL == 0:
                        ...
                        cleaned_search_results = append_track_data(
                            returned_data, track_name_artist_album, track_id, LIMIT
                        )
                        track_name_artist_album = cleaned_search_results[
                            "track_name_artist_album"
                        ]
                    ...
                    messages.error(
                        request,
                        "ERROR at search_track_name: Unknown error fetching track names.",
                    )
                    return redirect("room", room_id=room.id)
        ...
        return render(
            request,
            "search_track_name.html",
            {"form": form, "playlist_id": playlist_id},
        )
...
def search_results(request, cleaned_search_results, playlist_id):
    ...
    if len(cleaned_search_results) == 1:
        ...
        return render(
            request,
            "search_results.html",
            {"context": context, "playlist_id": playlist_id},
        )
    else:
    ...
    if Track.objects.filter(playlist=playlist).filter(track_id=track_id).count() == 0:
        try:
            ...
            spotify.user_playlist_add_tracks(
                user=user_id, playlist_id=playlist_id, tracks=[track_id], position=None
            )
            ...
            # }
            messages.error(
                request, request["error"]["status"] + " " + request["error"]["message"]
            )
            return redirect("room", room_id=room.id)
        else:
