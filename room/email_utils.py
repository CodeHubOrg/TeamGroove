...
def send_invitation(to_email, code, room):
    ...
    html_content = render_to_string(
        ...
        {"invitation_code": code, "room": room, "accept_url": accept_url},
    )
...
def send_invitation_accepted(room, invitation):
    ...
    html_content = render_to_string(
        "email_accepted_invitation.html", {"room": room, "invitation": invitation}
    )

    msg = EmailMultiAlternatives(
        subject, text_content, from_email, [room.created_by.email]
    )
    msg.attach_alternative(html_content, "text/html")
    msg.send()
